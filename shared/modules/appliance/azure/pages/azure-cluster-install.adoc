...
. *Review configuration options* Refer to xref:/appliance/azure/configuration-options.adoc[Azure configuration options] for detailed instance specs.
. *Create the instance* Refer to xref:/appliance/azure/launch-an-instance.adoc[Set up Azure for ThoughtSpot] to create and launch your instance.
...
. *Configure nodes* Refer to xref:/appliance/azure/installing-azure.adoc[Configure ThoughtSpot nodes in Azure] to configure your nodes.
...
. Make this change to each node to ensure that you can log back in:
+
a.
Open the grub file  `/update/etc/default/grub`:
