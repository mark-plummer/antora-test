...
Contact xref:/admin/misc/contact.adoc#[ThoughtSpot Support] to find out if your specific cluster size will benefit from the GCS storage option.
...
+
image::/images/gcp-selectproj.png[Select a project]

. Under *Select From*, pick your company's project.
. Go to the Compute Engine dashboard.
+
image::/images/gcp-computeenginedash.png[Go to the Compute Engine Dashboard]
...
+ Refer to xref:/appliance/gcp/configuration-options.adoc#thoughtspot-gcp-instance-types[ThoughtSpot GCP instance types] to determine the number of vCPU cores your cluster needs.
. Specify your memory requirements and CPU platform.
Refer to xref:/appliance/gcp/configuration-options.adoc#thoughtspot-gcp-instance-types[ThoughtSpot GCP instance types] to determine the memory your cluster needs.
+
...
|===
+
image::/images/gcp-machineconfig.png[Specify machine configuration]
+
image::/images/gcp-3-preferred-CPUs.png[Preferred CPU platforms]
...
+
image::/images/gcp-4-change-boot-disk.png[Change boot disk]
+
b.
...
+
image::/images/gcp-selecttsimage.png[Select the latest ThoughtSpot image]
+
...
+
image::/images/gcp-6-save-boot-disk-expand-mgmt.png[Advanced configuration options]

. Attach two 1 TB SSD drives for data storage.
Refer to xref:/appliance/gcp/configuration-options.adoc#vms-with-persistent-disk-only-storage[SSD-only persistent storage].
If you are using GCS, attach only 1 SSD drive, with 500 GB instead of 1 TB.
Refer to xref:/appliance/gcp/configuration-options.adoc#vms-with-persistent-disk-and-google-cloud-storage[GCS and SSD persistent storage].
+
a.
Click the *Disks* tab, and click *Add new disk*.
+
image::/images/gcp-7-advanced-disk-config.png[Add new disk]
+
You can select or unselect the *Deletion rule*, depending on your preferences.
+
b.
Configure the following settings for each disk.
Refer to xref:/appliance/gcp/configuration-options.adoc#vms-with-persistent-disk-and-google-cloud-storage[ThoughtSpot GCP instance types] to determine the size in GB when you have GCS.
...
+
image::/images/gcp-8-advanced-blank-disk-config.png[Configure your disk]
...
+
image::/images/gcp-setnetworkinterface.png[Set your network interface]
+
...
Refer to xref:/admin/setup/firewall-ports.adoc#required-ports-for-cluster-communication[network policies] for more information on what ports to open for intracluster operation, so that your clusters can communicate.
...
To install your ThoughtSpot cluster, complete the installation process outlined in xref:/appliance/gcp/installing-gcp.adoc[Installing ThoughtSpot in GCP].

== Related information

https://cloud.google.com/compute/docs/disks/gcs-buckets[Connecting to Google Cloud Storage buckets]

xref:/admin/loading/use-data-importer.adoc#loading-data-from-a-gcp-gcs-bucket[Loading data from a GCP GCS bucket]
