...
+
image::/images/filter-in-search-bar.png[Filter from the search bar]
+
You can also use date- and time-related keywords like `yesterday`, `after`, and `next month` to  filter your search.
To see more keywords, refer to the xref:/reference/keywords.adoc[keyword reference].
...
You can find out more about xref:/complex-search/pinboard-filters.adoc[pinboard filters in the pinboards section].

== Where filters appear in ThoughtSpot

As you have seen with search, filters appear in grey boxes in the search bar.

image::/images/search-bar-basics.png[Search bar with filters]
...
image::/images/filter-list-location.png[Filters appear under the title]
...
When adding a filter from the ellipsis icon image::/images/icon-ellipses.png[more options menu icon], in the column header or by clicking on a chart axis, numeric columns and text columns provide you with the ability to include or exclude values, and a checkbox selector for the values.
...
You can xref:/complex-search/create-bulk-filter.adoc[create a bulk filter] by pasting a list of values, separated by commas, semicolons, new lines, or tabs, into the bulk filter box.
