...
For more details, see https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Subnets.adoc[VPCs and Subnets] in Amazon's AWS documentation.
...
| &#10063;
| AWS security groups.
For required open ports, see xref:/appliance/firewall-ports.adoc[Network Policies].

| &#10063;
| AWS VM instances.
For instance type recommendations, see xref:/appliance/aws/configuration-options.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types].
...
|===
+
NOTE: The AMI is based in the N.California region.
...
. Choose the appropriate EC2 instance type: See xref:/appliance/aws/configuration-options.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types] for help choosing the correct instance type for your cluster.
...
Refer to xref:/appliance/aws/configuration-options.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types] for recommended nodes for a given data size.
...
Contact xref:/admin/misc/contact.adoc[ThoughtSpot Support] to find out if your specific cluster size can benefit from the S3 storage option.
...
+
image::/images/navigate_to_ec2_dashboard.png[]
...
+
image::/images/aws-choose-ami.png[Select the ThoughtSpot AMI]

. On the *Choose an Instance Type* page, select a ThoughtSpot-supported instance type.
(See xref:/appliance/aws/configuration-options.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types].)
...
For specific storage requirements, refer to xref:/appliance/aws/configuration-options.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types].
+
image::/images/aws-add-storage.png[Add storage volumes]
+
[cols="2,98",frame=none,grid=none]
|===
| *1*
...
+
Refer to xref:/appliance/firewall-ports.adoc[network policies].
...
Before installing a ThoughtSpot cluster, an administrator must xref:/appliance/aws/aws-prepare-vms.adoc[prepare the VMs.]
