...
If you completed ThoughtSpot's xref:/site-survey.pdf[site survey] form and returned it to xref:/appliance/contact.adoc[ThoughtSpot Support] before ThoughtSpot shipped the appliance, the appliance may be pre-configured for your network environment and ready to install and connect to your network.
...
. If the output shows READY, and looks like the xref:/appliance/hardware/smc-cluster-install.adoc#check-cluster-health[cluster status output] in the next article, your appliance is configured.
. Skip to xref:/appliance/hardware/smc-cluster-install.adoc#step-3-finalize-installation[Finalize installation].
...
Edit only the parts of the nodes.config file that are explicitly discussed in xref:/appliance/hardware/parameters-nodesconfig.adoc[Parameters of `nodes.config`].
...
+ If the command returns an error, refer to xref:/appliance/hardware/smc-cluster-install.adoc#set-config-error-recovery[set-config error recovery].
...
Next, xref:/appliance/hardware/smc-cluster-install.adoc[install your cluster.]
